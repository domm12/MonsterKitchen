...
void Sprite::shift(int dx, int dy){
    pos_x += dx;
    pos_y += dy;
}

void Sprite::ch_pos(int new_x, int new_y){
    pos_x = new_x;
    pos_y = new_y;
}
...
Controller::Controller():facing(UP){
    source.x = facing * BLOCK_SIZE;
    source.y = 0;
    source.width = BLOCK_SIZE;
    source.height = BLOCK_SIZE;
}

void Controller::draw(){
    DrawTextureRec(texture, source, {(float)pos_x, (float)pos_y}, WHITE);
}
